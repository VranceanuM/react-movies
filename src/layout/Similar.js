  ...
  componentDidMount() {
    ...
    axios
      .get(
        `https://api.themoviedb.org/3/movie/${
          this.props.match.params.id
        }/similar?api_key=${process.env.REACT_APP_MM_KEY}&language=en-US&page=1`
      )
      .then(res => {
        this.setState({
          similarMovies: res.data.results.slice(0, 6)
        });
      })
  ...
  render() {
    ...
    return (
      <React.Fragment>
        ...
        <SimilarMovieCard similarMovies={similarMovies} heading={heading} />
      </React.Fragment>
    );
  }
  _onReady(event) {
